...
int main() {
	...
	// PoE parameters
	// Joint angles
	float thetas[arm.N_JOINTS] = {M_PI/2, M_PI/3, 0, M_PI/6};
...
void PoE(float *thetas, float *points, float *omegas, linalg::Matrix &result, int N) {
	...
	for (int i = 0; i < N; i++) {
		...
		// Compute R_term_3
		linalg::matScalarMul(skew_omega, (1 - cos(thetas[i])), R_term_3);
		...
		linalg::printMat(v_T, "transposed v");
